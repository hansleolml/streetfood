...
<div class="col-md-8">
		...
		<div class="container-fluid">
			<div class="panel panel-primary">
			  	<div class="panel-heading">
			    	<h3 class="panel-title">Consulta de Ganancias por Chef </h3>
			  	</div>
					...
					<form class="form-horizontal" role="form" method="POST" action="{{url('reporte/chef')}}">
						{{csrf_field()}}
					  	<div class="form-group">
					    	...
					    	<div class="col-sm-10">
						    	<input type="date" class="form-control" name="fecha_inicio" value="{{$fecha_inicio}}" id="fecha_inicio" required>
						    </div>
					  	</div>
					  	<div class="form-group">
	...
	<div class="row" >
		<div class="col-md-12">
			...
			<table id="example" class="display nowrap" style="width:100%">
				...
				<tbody>
					...
					<tr>
						...
						@endforeach
						@foreach($user as $us1)
							@if($us1->id==$reser->id_clienteFO)
							<td>{{$us1->name}} {{$us1->apellidos}}</td>
							@endif
						@endforeach
						<td>{{$reser->cantidad}}</td>
						<td>{{$reser->cantidad * $pro->precio}}</td>
			...
			<h2 id="titulo">Reporte de Reservas por citas del dia {{$fecha_inicio}} al {{$fecha_fin}} </h2>
...
var chart = AmCharts.makeChart("chartdiv", {
  ...
  "dataProvider": [
  {
  ...
  },
   {
    ...
    "pedido": cellVal_cant2,
    "color": "#FF6600"
  },
  {
    "categoria": "Jose Palomino",
    "pedido": cellVal_cant3,
    "color": "#FF6600"
  },
  ],
  "valueAxes": [{
    "axisAlpha": 0,
    "position": "left",
    "title": "Cantidad de dinero ganado"
  }],
  "startDuration": 1,
  "graphs": [{
